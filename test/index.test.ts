import test from 'ava';
import * as StellarSdk from 'stellar-sdk';
import { encrypt, decrypt } from '../src';

const aliceKeys = StellarSdk.Keypair.random();
const bobKeys = StellarSdk.Keypair.random();

test('encrypt + decrypt', async (t) => {
    const data = Buffer.from('', 'hex');
    const nonce = data.slice(0, 12);
    const res0 = Buffer.from(await encrypt(aliceKeys, bobKeys.publicKey(), data,  nonce));
    const res1 = Buffer.from(await decrypt(bobKeys, aliceKeys.publicKey(), res0, nonce));
    t.true(data.compare(res1) === 0);
});
